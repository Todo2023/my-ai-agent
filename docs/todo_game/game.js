(() => {
  ...
  // --- お品書き ---------------------------------------------------------
  const MENU = [
    { id: "yakiniku", name: "焼肉",   color: "#b06a3f", deep: "#6d3a1c", lite: "#dba475" },
    { id: "ikura",    name: "いくら", color: "#f0913a", deep: "#a35510", lite: "#f9c184" },
    { id: "tsumami",  name: "つまみ", color: "#cbb489", deep: "#87703f", lite: "#e6dcc2" },
    { id: "parfait",  name: "パフェ", color: "#ef7fa8", deep: "#a12f5c", lite: "#f8b6cd" },
    { id: "wine",     name: "ワイン", color: "#9a5fc9", deep: "#552a83", lite: "#c7a3e8" },
  ];
  // トドの苦手なもの。注文には出ないが皿には入っている。
  const NEGI = { id: "negi", name: "ネギ", color: "#6fbf63", deep: "#2f7331", lite: "#a9dda3" };
  ...
  function drawIcon(kind, x, y, size) {
    ...
    if (kind.id === "yakiniku") {
      ...
      ctx.strokeStyle = INK;
      ctx.lineWidth = 0.06;
      for (const yy of [0.3, 0.42]) {
        ctx.beginPath(); ctx.moveTo(-0.46, yy); ctx.lineTo(0.46, yy); ctx.stroke();
      }
      for (const xx of [-0.34, -0.11, 0.11, 0.34]) {
        ctx.beginPath(); ctx.moveTo(xx, 0.24); ctx.lineTo(xx, 0.48); ctx.stroke();
      }
      ctx.fillStyle = INK;
      ctx.beginPath();
      ctx.moveTo(-0.4, -0.02);
      ctx.quadraticCurveTo(-0.44, -0.36, -0.06, -0.38);
      ctx.quadraticCurveTo(0.36, -0.4, 0.4, -0.1);
      ctx.quadraticCurveTo(0.44, 0.2, 0.02, 0.22);
      ctx.quadraticCurveTo(-0.36, 0.24, -0.4, -0.02);
      ctx.closePath();
      ctx.fill();
      // 焼き目
      ctx.strokeStyle = PAPER;
      ctx.lineWidth = 0.075;
      for (const [x1, y1, x2, y2] of [[-0.26, 0.06, 0.02, -0.24], [-0.04, 0.14, 0.24, -0.16], [0.14, 0.18, 0.34, -0.04]]) {
        ctx.beginPath(); ctx.moveTo(x1, y1); ctx.lineTo(x2, y2); ctx.stroke();
      }
    } else if (kind.id === "ikura") {
      // いくらの軍艦
      ctx.fillStyle = PAPER;                     // シャリ
      ctx.beginPath();
      ctx.moveTo(-0.3, -0.02); ctx.lineTo(0.3, -0.02);
      ctx.quadraticCurveTo(0.34, 0.36, 0, 0.36);
      ctx.quadraticCurveTo(-0.34, 0.36, -0.3, -0.02);
      ctx.closePath(); ctx.fill();
      ctx.fillStyle = INK;                       // 海苔
      ctx.beginPath();
      ctx.moveTo(-0.34, -0.16); ctx.lineTo(0.34, -0.16); ctx.lineTo(0.32, 0.3);
      ctx.quadraticCurveTo(0, 0.4, -0.32, 0.3);
      ctx.closePath(); ctx.fill();
      // 粒
      ctx.fillStyle = PAPER;
      for (const [bx, by] of [[-0.2, -0.2], [0, -0.26], [0.2, -0.2], [-0.1, -0.13], [0.1, -0.13]]) {
        ctx.beginPath(); ctx.arc(bx, by, 0.098, 0, Math.PI * 2); ctx.fill();
      }
      ctx.fillStyle = INK;
      for (const [bx, by] of [[-0.2, -0.2], [0, -0.26], [0.2, -0.2], [-0.1, -0.13], [0.1, -0.13]]) {
        ctx.beginPath(); ctx.arc(bx + 0.028, by - 0.028, 0.03, 0, Math.PI * 2); ctx.fill();
      }
    } else if (kind.id === "tsumami") {
      // 豆皿と乾き物
      ctx.fillStyle = INK;
      ctx.beginPath();
      ctx.moveTo(-0.44, 0.04);
      ctx.quadraticCurveTo(-0.44, 0.42, 0, 0.42);
      ctx.quadraticCurveTo(0.44, 0.42, 0.44, 0.04);
      ctx.closePath(); ctx.fill();
      ctx.fillStyle = PAPER;
      ctx.beginPath();
      ctx.moveTo(-0.34, 0.1);
      ctx.quadraticCurveTo(-0.33, 0.32, 0, 0.32);
      ctx.quadraticCurveTo(0.33, 0.32, 0.34, 0.1);
      ctx.closePath(); ctx.fill();
      ctx.fillStyle = INK;                       // 盛られた豆
      for (const [bx, by] of [[-0.2, -0.02], [0, -0.1], [0.2, -0.02], [-0.1, -0.18], [0.1, -0.18]]) {
        ctx.beginPath(); ctx.ellipse(bx, by, 0.11, 0.085, 0.2, 0, Math.PI * 2); ctx.fill();
      }
      ctx.strokeStyle = PAPER;
      ctx.lineWidth = 0.03;
      for (const [bx, by] of [[-0.2, -0.02], [0, -0.1], [0.2, -0.02]]) {
        ctx.beginPath(); ctx.moveTo(bx - 0.06, by); ctx.lineTo(bx + 0.06, by); ctx.stroke();
      }
    } else if (kind.id === "parfait") {
      // パフェ
      ctx.fillStyle = INK;
      ctx.beginPath();
      ...
      ctx.lineTo(-0.13, 0.3);
      ctx.closePath(); ctx.fill();
      ctx.fillRect(-0.035, 0.28, 0.07, 0.1);
      ctx.beginPath(); ctx.ellipse(0, 0.42, 0.19, 0.055, 0, 0, Math.PI * 2); ctx.fill();
      // 中身の層
      ctx.fillStyle = PAPER;
      ctx.fillRect(-0.19, -0.03, 0.38, 0.07);
      ctx.fillRect(-0.155, 0.11, 0.31, 0.06);
      // 生クリーム
      ctx.fillStyle = INK;
      ctx.beginPath();
      ctx.arc(-0.14, -0.2, 0.13, 0, Math.PI * 2);
      ctx.arc(0.05, -0.28, 0.16, 0, Math.PI * 2);
      ctx.arc(0.2, -0.17, 0.11, 0, Math.PI * 2);
      ctx.fill();
      // さくらんぼとウエハース
      ctx.beginPath(); ctx.arc(0.06, -0.44, 0.085, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = INK;
      ctx.lineWidth = 0.035;
      ctx.beginPath(); ctx.moveTo(0.08, -0.5); ctx.quadraticCurveTo(0.2, -0.56, 0.24, -0.44); ctx.stroke();
      ctx.fillStyle = PAPER;
      ctx.save();
      ctx.translate(-0.24, -0.34); ctx.rotate(-0.34);
      ctx.fillRect(-0.045, -0.13, 0.09, 0.28);
      ctx.restore();
    } else if (kind.id === "wine") {
      // ワイングラス
      ctx.fillStyle = INK;
      ctx.beginPath();
      ctx.moveTo(-0.26, -0.4);
      ctx.lineTo(0.26, -0.4);
      ctx.quadraticCurveTo(0.26, 0.02, 0, 0.06);
      ctx.quadraticCurveTo(-0.26, 0.02, -0.26, -0.4);
      ctx.closePath(); ctx.fill();
      ctx.fillRect(-0.035, 0.04, 0.07, 0.3);
      ctx.beginPath(); ctx.ellipse(0, 0.38, 0.22, 0.06, 0, 0, Math.PI * 2); ctx.fill();
      // 注がれた面
      ctx.fillStyle = PAPER;
      ctx.beginPath();
      ctx.moveTo(-0.225, -0.22);
      ctx.lineTo(0.225, -0.22);
      ctx.lineTo(0.207, -0.13);
      ctx.lineTo(-0.207, -0.13);
      ctx.closePath(); ctx.fill();
      ctx.globalAlpha = 0.55;
      ctx.beginPath(); ctx.ellipse(-0.11, -0.31, 0.045, 0.07, -0.2, 0, Math.PI * 2); ctx.fill();
      ctx.globalAlpha = 1;
    } else {
      // ネギ（苦手）
      ctx.save();
      ctx.rotate(-0.42);
      ctx.fillStyle = PAPER;                     // 白い根元
      ctx.beginPath();
      ctx.moveTo(-0.08, 0.06); ctx.lineTo(0.08, 0.06); ctx.lineTo(0.07, 0.46);
      ctx.quadraticCurveTo(0, 0.52, -0.07, 0.46);
      ctx.closePath(); ctx.fill();
      ctx.strokeStyle = PAPER;
      ctx.lineWidth = 0.028;
      for (const yy of [0.5, 0.54]) { ctx.beginPath(); ctx.moveTo(-0.05, yy); ctx.lineTo(0.05, yy + 0.03); ctx.stroke(); }
      ctx.fillStyle = INK;                       // 青い葉
      ctx.beginPath();
      ctx.moveTo(-0.08, 0.1); ctx.lineTo(0.08, 0.1);
      ctx.quadraticCurveTo(0.16, -0.2, 0.09, -0.5);
      ctx.quadraticCurveTo(0.02, -0.3, 0, -0.16);
      ctx.quadraticCurveTo(-0.03, -0.32, -0.12, -0.5);
      ctx.quadraticCurveTo(-0.16, -0.2, -0.08, 0.1);
      ctx.closePath(); ctx.fill();
      ctx.restore();
    }
    ctx.restore();
  ...
  const TUT = [
    ...
    { focus: null,    wait: "tap",   lines: () => ["ネギだけは大の苦手。注文には出ませんが、", "開けてしまうとひどく機嫌を損ねます。"] },
  ...
  function newGame() {
    S = {
      ...
      flyers: [],
      puffs: [],
      todo: { chew: 0, face: 0, spit: 0, blink: 0, roll: 0, wiggle: 0, yuck: 0 },
      hint: null,
      overT: 0,
      noren: { part: 0, sway: 0 },
      tut: null,
  ...
  function pick(hazard = true) {
    if (hazard && Math.random() < 0.14) return NEGI;
  ...
  function openPlate(p) {
    ...
    const hit = p.kind.id === S.want.id;
    if (tutorial) {
      S.tut.result = hit
        ? ["当たり。トドが喜んでお尻を振ります。", "続けて当てるほど点数が伸びます。"]
        : p.kind.id === "negi"
          ? ["ネギ。トドの大の苦手です。", "顔をしかめて、ごきげんがどんと下がります。"]
          : ["はずれ。でも蓋は開いたままです。", "覚えておけば、次に注文されたとき出せます。"];
    }

    if (hit) {
      eat(p);
    } else if (p.kind.id === "negi") {
      S.combo = 0;
      S.mood = Math.max(0, S.mood - (known ? 0.2 : 0.14));
      S.todo.yuck = YUCK_TIME;
      S.todo.face = -1;
      puff(TODO.x, TODO.y - 20, NEGI.color, 14);
      say(known ? "だからネギは……" : "うええ、ネギ");
      beep(150, 0.32, "sawtooth", 0.05, 60);
  ...
  function update(dt) {
    ...
    td.wiggle = Math.max(0, td.wiggle - dt);
    td.yuck = Math.max(0, td.yuck - dt);
  ...
  const WIGGLE_TIME = 0.9, YUCK_TIME = 1.3;

  // 後ろ姿。おあいそのときだけ出る。
  function drawTodoBack(walk) {
    const body = ctx.createLinearGradient(0, -50, 0, 62);
    body.addColorStop(0, "#8b7568");
    body.addColorStop(1, "#584740");
    ctx.fillStyle = body;
    ctx.beginPath(); ctx.ellipse(0, 18, 58, 44, 0, 0, Math.PI * 2); ctx.fill();

    ctx.strokeStyle = "rgba(42,26,24,0.22)";
    ctx.lineWidth = 3;
    ctx.beginPath(); ctx.moveTo(0, -6); ctx.lineTo(0, 48); ctx.stroke();

    const flap = REDUCED ? 0 : Math.sin(walk * 26) * 0.3;
    ctx.fillStyle = "#4e3f38";
    ctx.beginPath(); ctx.ellipse(-54, 42, 19, 10, -0.3 + flap, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(54, 42, 19, 10, 0.3 - flap, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(0, 59, 15, 9, 0, 0, Math.PI * 2); ctx.fill();

    ctx.fillStyle = "#95806f";
    ctx.beginPath(); ctx.ellipse(0, -20, 42, 36, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "rgba(42,26,24,0.16)";
    ctx.beginPath(); ctx.ellipse(-29, -27, 8, 5, -0.4, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(29, -27, 8, 5, 0.4, 0, Math.PI * 2); ctx.fill();

    ctx.strokeStyle = "rgba(242,234,217,0.3)";
    ctx.lineWidth = 1.2;
    ctx.beginPath(); ctx.moveTo(-40, -6); ctx.lineTo(-49, -9); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(40, -6); ctx.lineTo(49, -9); ctx.stroke();
  }

  function drawTodoFront(t, mouthOpen, showOrder) {
    const yuck = t.yuck > 0;
    ...
    const flap = REDUCED ? 0 : Math.sin(S.t * 3) * 0.08 + t.chew * 0.5 + (t.wiggle > 0 ? Math.sin(t.wiggle * 26) * 0.3 : 0);
    ...
    if (yuck) {
      ...
      ctx.beginPath();
      ctx.moveTo(-19, 4);
      ctx.quadraticCurveTo(-9, -6, 0, 2);
      ctx.quadraticCurveTo(9, 10, 19, 0);
      ctx.stroke();
      // しわ
      ctx.lineWidth = 2;
      ...
      const want = showOrder ? S.want : null;
      if (want && !rolling) {
        ctx.save();
    ...
    // 目
    const happy = t.face > 0.3 || t.wiggle > 0;
    const bad = !yuck && (t.face < -0.3 || S.phase === "over");
    ...
    for (const ex of [-27, 27]) {
      ...
      if (yuck) {
        ...
        ctx.lineWidth = 3.4;
        ctx.beginPath();
        ctx.moveTo(ex - 8 * -sign, -32);
        ctx.lineTo(ex + 2 * -sign, -37);
        ctx.lineTo(ex + 10 * -sign, -31);
        ctx.stroke();
        ctx.lineWidth = 2.6;
        ctx.beginPath();
        ctx.moveTo(ex - 10 * -sign, -46);
        ctx.lineTo(ex + 9 * -sign, -41);
        ctx.stroke();
      } else if (happy || bad || blinking) {
        ctx.strokeStyle = "#2a1a18";
        ctx.lineWidth = 3;
        ctx.beginPath();
    ...
    if (yuck) {
      ...
      ctx.beginPath(); ctx.ellipse(-32, -14, 9, 6, 0, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.ellipse(32, -14, 9, 6, 0, 0, Math.PI * 2); ctx.fill();
      ctx.globalAlpha = 1;
    } else if (S.mood > 0.7 && S.phase !== "over") {
  ...
  function drawTodo() {
    ...
    const playing = S.phase === "play" || S.phase === "tutorial";

    // ふてくされて後ろを向き、のっそり帰っていく
    let dx = 0, dy = bob, scale = 1, alpha = 1, turn = 0, walk = 0;
    if (S.phase === "over") {
      turn = Math.min(1, S.overT / 0.55);
      walk = Math.max(0, Math.min(1, (S.overT - 0.62) / 2.0));
      const ease = walk * walk * (3 - 2 * walk);
      dx = -ease * 340;
      dy = REDUCED ? 0 : Math.sin(S.overT * 13) * 4 * (walk > 0 && walk < 1 ? 1 : 0);
      scale = 1 - ease * 0.22;
      alpha = 1 - Math.max(0, (walk - 0.72) / 0.28);
    } else if (t.roll > 0) {
      const k = 1 - t.roll / ROLL_TIME;
      dx = Math.sin(k * Math.PI) * 74 * (S.rollDir || 1);
      dy = -Math.abs(Math.sin(k * Math.PI * 2)) * 14;
    } else if (t.yuck > 0 && !REDUCED) {
      dx = Math.sin(t.yuck * 40) * 5;              // ぶるっと身震い
    }

    // 影は回さない
    ctx.save();
    ctx.globalAlpha = (t.roll > 0 ? 0.5 : 1) * alpha;
    ctx.fillStyle = "rgba(0,0,0,0.35)";
    ctx.beginPath();
    ctx.ellipse(TODO.x + dx, TODO.y + 62, 62 * scale, 12 * scale, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();

    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.translate(TODO.x + dx, TODO.y + dy);
    ctx.scale(scale, scale);

    if (t.roll > 0) {
      const k = 1 - t.roll / ROLL_TIME;
      const spin = REDUCED ? 0 : k * Math.PI * 4;
      ctx.translate(0, 18);
      ctx.rotate(spin * (S.rollDir || 1));
      ctx.translate(0, -18);
    } else if (t.wiggle > 0 && !REDUCED) {
      // 喜んでお尻を振る。首を軸にすると尻が振れる。
      const amp = 0.11 * Math.min(1, t.wiggle / (WIGGLE_TIME * 0.5));
      ctx.translate(0, -22);
      ctx.rotate(Math.sin(t.wiggle * 26) * amp);
      ctx.translate(0, 22);
    }

    if (S.phase === "over") {
      if (turn < 0.5) {
        ctx.scale(Math.max(0.02, Math.cos(turn * Math.PI)), 1);
        drawTodoFront(t, mouthOpen, false);
      } else {
        ctx.scale(Math.max(0.02, -Math.cos(turn * Math.PI)), 1);
        drawTodoBack(walk);
      }
    } else {
      drawTodoFront(t, mouthOpen, playing);
    }
    ctx.restore();

    // 振り向きぎわの「ふんっ」
    if (S.phase === "over" && S.overT > 0.35 && S.overT < 1.5) {
      ctx.save();
      ctx.globalAlpha = Math.min(1, (1.5 - S.overT) * 1.6) * alpha;
      ctx.fillStyle = "#f3e2c4";
      ctx.font = "600 18px 'Hiragino Mincho ProN', 'Yu Mincho', serif";
      ctx.textAlign = "left";
      ctx.textBaseline = "middle";
      ctx.fillText("ふんっ", TODO.x + dx + 46 * scale, TODO.y - 52 * scale);
      ctx.restore();
    }
  }
