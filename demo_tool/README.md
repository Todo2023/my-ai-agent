...
入らないときは、返ってくる `reason` が場所を教えます。
KVには何も書かないので、何度試しても無料枠は減りません。確認用の予定は手で消してください。

**2026-09-03 に実際につまずいた2つ**

| 出たもの | 原因 | 直しかた |
| --- | --- | --- |
| `受け口が 401 を返しました` | デプロイの設定を変えた直後で、**まだ配られていなかった** | 数分待って、もう一度試す。設定（実行＝自分／アクセス＝全員）が正しければ作り直さなくてよい |
| `受け口が「合言葉が違います」と返しました` | Cloudflare 側の `CALENDAR_TOKEN` が Apps Script の `TOKEN` と食い違っていた | `wrangler secret put CALENDAR_TOKEN` を**対話で**やり直す。`"値" \| wrangler ...` の形は、余分な文字が混ざることがある |

`ok: true` は「受け口が応答した」ではなく「**予定を作れた**」を意味します。
Apps Script は断りも 200 番で返すため、Worker 側で中身まで読んで判定しています
（`toCalendar()`）。ここを番号だけで見ると、入っていないのに「送れた」と報告します。
