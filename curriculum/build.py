...
(function () {
  ...
  function send() {
    ...
    fetch(ASK_URL, {
      ...
      ask.value = '';
      /* 運営に通知が飛んだかどうかを、そのまま伝える。
         「送りました」とだけ出すと、届いていないときに誰も気づけない */
      askSay(res.d.delivered
        ? '送りました。24時間以内にお返事します'
...
def render_lesson(lesson: Lesson, prev: Lesson | None, nxt: Lesson | None) -> str:
    ...
    <div class="studio-inner">
      ...
      <div class="asker">
        ...
        <p class="writer-note">運営に直接届きます。名前は空でも構いません。<b>質問は24時間以内に回答します。</b>答えはSlackでお返しします。</p>
      </div>
    </div>
  </section>'''
    else:
