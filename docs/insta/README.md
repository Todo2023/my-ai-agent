...
| `tools/kensa.mjs` | 出す前の機械検品。**何も書き換えない。読むだけ** |
| `tools/uriage.mjs` | 週1で5つの数字を書き留め、**直す場所を1つ出す**。書き換えるのは `uriage.json` だけ |
...
```bash
node insta/tools/kensa.mjs <下書きのファイル>
```

**AIを呼ばないので費用はゼロ。**文字を数えて、決まった形を探すだけ。
「だめ」が1つでもあれば終了コード1で終わる。**出さないこと。**

| 見るもの | 落とす理由 |
| --- | --- |
| 本文2,200文字／ハッシュタグ30個 | 超えると Instagram に弾かれる |
| 書いてはいけない言葉 | 「必ず儲かる」など断定、いま事実でない「無料」、書きかけの「要記入」 |
| メールアドレス・電話番号 | 個人情報を出さない |
| 出どころのない数字 | 数字を出すなら出典を添える。無いなら消す |
| リンクが2本以上 | 行き先を1つに絞る。分岐を作ると誰も進まない |
| 最初の2行の弱さ | 数字か、読み手の思い込みを否定する形が無いと止まらない |
| 行き先が無い | 伸びても次に行く場所が無ければ売上にならない |

**作る側とは別の道具にしてある。** 同じ頭で確かめると、自分の文章を自分で褒めて終わる。

道具自体が壊れていないかは、これで確かめる。
...
| 数字 | どこから写すか |
| --- | --- |
| 表示・保存・プロフィール閲覧 | Instagram のインサイト |
| リンククリック | プロフィールのリンク先の数字 |
| 購入 | **note の売上画面**（note には公開APIが無いので手で写す） |

**5つ写すだけ。週に1回。** AIを呼ばないので費用ゼロ、無料枠も使わない。

### 直す場所は1つだけ出す

```
表示されても保存されないなら  → 投稿の中身を直す
プロフィールへ来ないなら      → 投稿の締めを直す
リンクを押さないなら          → プロフィールを直す
押しても買われないなら        → note の記事か値段を直す
```

**勘で全部作り直さない。**人が一番減っている1か所だけを直す。

### 段ごとに「落ちて当たり前」の量が違う

生の割合をそのまま比べると、**毎回「表示→保存」が最悪になります。**
そこはもともと数%しか残らないからです。同じ場所を直し続けることになる。

なので、比べるのは**その段の「普通」に対してどうか**。

- 最初は**出典のない目安**を使う（当てずっぽうの初期値）
- **3週ぶん貯まると、自分の実績の中央値に自動で切り替わる**
