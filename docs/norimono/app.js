...
const CHARAS = [
  ...
  { name: "ロケット", file: "rocket", fly: true, launch: true, encore: undefined, body: "#f4f6fb", roof: "#e5484d", win: "#8ecfff", kind: "rocket", label: "ろけっと",
    ...
    tune: [["G4",0.5],["C5",1.5],["C5",0.5],["B4",0.5],["A4",0.5],["B4",1.5],
      ...
      ["E5",0.5],["D5",0.5],["C5",1],["G4",1],["A4",1],["B4",1],["C5",2],
      ["G4",0.5],["C5",1.5],["E5",1],["G5",2]] },
];
...
function tuneSeq(c) {
  ...
  return c.tune.concat(c.tune); // 1曲は短いので、2回くり返す
}
...
function showZoom(c) {
  ...
  walkTimer = setTimeout(() => {
    ...
    const cycle = c.launch ? 2.4 : 4;  // ロケットは短く、勢いよく
    face.style.animationDuration = `${cycle}s`;
    face.style.animationIterationCount = String(Math.max(1, Math.round(songLen / cycle)));
