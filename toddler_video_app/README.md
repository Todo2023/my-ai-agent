...
一度オンラインで開いたあとなら、通信がなくても**画面は開く**（`sw.js` が一式をキャッシュする）。
ルーレットも回せる。ただし**動画の再生だけは通信が必要**。YouTube への通信は
Service Worker では触らないので、キャッシュには残らない。

| | 圏外のとき |
| --- | --- |
| アプリの起動・ルーレット | 動く |
| フィルムのサムネイル | 絵文字に変わる |
| 動画の再生 | できない。「いまは どうがが みられません」と出す（黒い画面で待たせない） |
...
`videos.js` を変えたら `sw.js` の `CACHE` の版番号（`todo-wanwan-v11`）を1つ上げる。
