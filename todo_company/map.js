...
(function () {
  ...
  function renderPanel(n) {
    ...
    if (n.post) {
      html += '<div class="post">'
        ...
        + '<div class="post__row">'
        + '<button type="button" class="btn" id="copy">投稿文をコピー</button>'
        + '<span class="post__count" id="count"></span>'
        + '</div>'
        + '<p class="post__note">いまはコピーまで。X や Threads へそのまま発信するところは、'
        + 'あとで投稿用のつなぎ込みを足す回で動くようにします。</p>'
        + '</div>';
    } else if (n.cat === 'blank') {
    ...
    if (ta) {
      ...
      document.getElementById('copy').addEventListener('click', function () { copy(ta); });
    }
  }
  ...
  function count(ta) {
    ...
    el.textContent = len + ' 文字 / 140';
    el.setAttribute('data-over', String(len > 140));
  }

  function copy(ta) {
    function done() { toast('投稿文をコピーしました'); }
    function manual() {
      ta.focus();
      ta.select();
      toast('選びました。長押し／Ctrl+C でコピーしてください');
    }
