...
export async function submitArticle(draft, topics) {
  ...
  const row = {
    ...
    topics,
    body_free: String(draft.src).trim(),
    is_pr: draft.pr === true,
    status: "review",
  };
  const out = await api("/rest/v1/works", {
    method: "POST",
